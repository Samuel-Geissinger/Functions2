...
// CHECK YOUR ANSWER
 ...
 multiply(4, 3, answer => {
   console.log('The answers is ' + answer) //should console.log 12
 })
...
// CODE HERE 
const contains = (arr, newName, callback) => arr.includes(newName) ? callback(true) : callback(false); 
...
// CODE HERE
uniq(names, uniqArr => console.log(`The new names array with all the duplicate items remoced is ${uniqArr}`))
...
// CODE HERE 
const each = (arr, callback) => arr.forEach( (elem, index) => callback( elem, index)  );
...
// CODE HERE 
const getUserById = (arr, id, cb) => { arr.map(elem => {if(elem.id === id) cb(elem)} )}
...
// CODE HERE
const addNUMBER = addingFactory(2.71);
